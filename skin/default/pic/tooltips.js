...
this.Link=$(this.options.Link);this.ImageLink=$("#"+this.options.ImageLink);};
PopUp.prototype={SetOptions:function(a){
	this.options={Marks:".load_list",Link:".mainright .pro_link",Wrapper:"pop",Pointer:"pointer",BigImage:"BigImage",ImageLink:"ImgLink",ImgWidth:263,ImgHeight:263,LoadingSrc:$("#BigImage").attr("src")};
...
}});b.Marks[c].onclick=function(){};})();}},LoadImage:function(){var b=this;for(var a=0;a<this.Marks.length;a++){this.ImgSrc.push($(this.Marks[a]).attr("name"));(function(){var c=a;$(b.Marks[c]).bind("mouseenter",function(){b.ImageLink.attr("href","#blank");b.BigImage.attr("src",$("#BigImage").attr("src"));
...
}}}this._img=new Image();},Start:function(){this.SetPosition();this.LoadImage();var b=this;for(var a=0;a<this.Marks.length;a++){$(this.Marks[a]).bind("mouseenter",function(){b.Wrapper.fadeIn("slow"); b.Pointer.fadeIn("slow"); }).bind("mouseleave",function(){b.Wrapper.css({display:"none"});
b.Pointer.css({display:"none"});});}this.Pointer.bind("mouseenter",function(){b.Wrapper.css({display:"block"});b.Pointer.css({display:"block"});}).bind("mouseleave",function(){b.Wrapper.fadeOut("slow"); b.Pointer.fadeOut("slow"); });this.Wrapper.bind("mouseenter",function(){b.Wrapper.css({display:"block"});
