...
<div class="bodytitle">
	...
	<div class="bodytitletxt">新闻分类管理</div>
</div>
 </td>
 </tr>
</table>
...
<a href="a_news.php?action=add&classid=<?=$id?>">添加新闻</a></td>
  </tr>
  </table>
  </div>
 ...
 ?>
  <tr  bgcolor="#FFFFFF">
   ...
   <tr   bgcolor="#FFFFFF">
	 ...
	 <IFRAME ID="uppic" SRC="<?=REMOTEUPLOAD?>?form=formadd&input=pic&path=systemImage" FRAMEBORDER="0"  SCROLLING="no" WIDTH="700" HEIGHT="35" ></IFRAME>
	 </td>
    </tr>
  <?
}
...
while($rows=fetch($rs))
{
  ...
  <tr  bgcolor="#FFFFFF">
	...
	<tr   bgcolor="#FFFFFF">
     <td colspan="4" align="left"><IFRAME ID="uppic" SRC="<?=REMOTEUPLOAD?>?form=formadd&input=pic<?=$index?>&path=systemImage" FRAMEBORDER="0"  SCROLLING="no" WIDTH="700" HEIGHT="35" ></IFRAME></td>
    </tr>
 <?
 $index++;
 }
...
function delete_save()
{
	...
	global $page;
	$id=getQuerySQL("id");
	$tree=fetchValue("select tree as v from @@@newsclass where id=$id","0");
...
function edit_save()
{
	...
	$sql=updateSQL($param,"@@@newsclass",$condition);
	query($sql);
