...
<div class="bodytitle">
	...
	<div class="bodytitletxt">产品分类分析</div>
</div>
 </td>
 </tr>
</table>
...
<table border="0" align="center" width="96%" cellpadding="1" cellspacing="1" class="tbtitle">
  ...
  <tr bgcolor="#FFFFFF">
      <td class="a_fen">
	   分类ID：
      <input name="classid" type="text" id="classid" value="<?=getRequest("classid")?>" style="width:80px"><span class="fontpadding">年份选择 ： 
...
$condition.= " and classid='" . getRequestInt("classid",0) . "'";
$pageurl.="&addtime=" . $addtime ;
$title = "分类 " . fetchValue("select name as v from @@@productclass where id=".getRequest("classid"),"分类已删除") . " 1月-12月的访问的分布情况" ;
?><br>
<table border="0" width="96%" align="center" cellpadding="1" cellspacing="1" class="tbtitle">
  ...
  <tr align="center"  bgcolor="#FFFFE9"  >
   <td width="4%"   >月份</td><td width="8%"  >访问次数</td>
...
<?
$total = fetchValue("select count(*) as v from @@@categorylog $condition",0);
if($total==0)
	$total=1;
$sql = "select month(addtime) as strmonth,count(*) as t from @@@categorylog  $condition group by month(addtime) order by strmonth desc";
$rs = query($sql);
emptyList($rs,10);
while($rows=fetch($rs))
{
...
<table width="96%" border="0" cellspacing="1" cellpadding="1" align="center" class="tbtitle" style="background:#FEDF63;">
  <tr  bgcolor="#FFFFE6">
    <td >请输入要生成进行细化分析的 产品分类的ID。例如： http://xxx.com/?2010-new-products-<span class="red">c520</span>.html 就输入 520</td>
  </tr>
</table>
<?
}
