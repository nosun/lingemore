...
function getAddressHTML(table,field,id,isnum,size,index)
{
	
	var obj= document.getElementById("" + field + index);
	var inputsize =  size || 5; 
	var value=$("#"+field + index).text();
	$(obj).unbind("dblclick");
	//alert($("#" + field + id).html()); 
	$("#" + field + index).html("");
	$("#" + field + index).append("<input type='text' size='"+inputsize+"'  id='input"+field+index+"' value='"+value+"' />");
	$("#" + field + index).append("<input type='button' value='修改' class='qianbibutton' onclick=\"changeAddress('"+table+"','"+field+"','"+id+"','"+isnum+"','"+inputsize+"','"+index+"')\" />");
	var input3=document.createElement("input");
	input3.type="button";
	input3.value="取消";
	input3.className="qianbibutton";
	$(input3).bind("mouseup",function(){ $("#" + field + index).html(value);  $(obj).bind("dblclick",function(){ getAddressHTML(table,field,id,isnum,size,index) });        } );
	$("#" + field + index).append($(input3));
...
function changeValue(table,field,id,isnum,size)
{
	...
	var src="../service/serviceForChangeValue.php?table=" + table + "&field=" + field + "&size=" + size + "&isnum=" + isnum + "&id=" + id + "&value=" + encodeURIComponent(value);
	includeJsFile("Ajax_change",src) 
}
function changeAddress(table,field,id,isnum,size,index)
{
	
	var value=document.getElementById("input" + field + index).value;
	if(isnum==1 && isNaN(value)  )
	{
		alert("提醒您:应该为数字！"); 
		return false;
	}
	var src="../service/serviceForChangeAddress.php?table=" + table + "&field=" + field + "&size=" + size + "&isnum=" + isnum + "&id=" + id + "&value=" + encodeURIComponent(value)+"&index="+index;
...
function changeAddressSuccess(flag,table,field,id,isnum,size,index)
{
	...
	obj.innerHTML=value;
	$(obj).unbind("dblclick");
	$(obj).bind("dblclick",function(){getAddressHTML(table,field,id,isnum,size,index);});
}

function showtips(obj)
{
	obj.value="提交中...,请勿重复点击提交";
