...
<div class="bodytitle">
	...
	<div class="bodytitletxt">搜索词管理</div>
</div>
 </td>
 </tr>
</table>
...
<table width="96%" border="0" cellspacing="1" cellpadding="1" align="center" class="tbtitle"  >
  <tr  bgcolor="#F2F4F6">
    <td    ><a href="?">搜索词列表</a> (该功能由前台客户点击搜索时自动收集而成,<span class="red weight">登入管理员后不列入统计</span>)</td>
  </tr>
</table><br />
<?php
...
if(getRequest("addtime"))
{
	...
	if(getRequest("timetype")=="y")
	{
		...
		$title = date("Y",strtotime(getRequest("addtime"))) . "年份 的搜索词" ;
	}
	...
	else if(getRequest("timetype")=="q")
	{
		...
		$title = date("Y",strtotime(getRequest("addtime"))) . "年" . $arrq[date("n",strtotime(getRequest("addtime")))-1] . " 的搜索词" ; 
	}
	else if(getRequest("timetype")=="m")
	{
		$timesql = "YEAR(addtime)=YEAR('" . getRequest("addtime") . "') and MONTH(addtime)=MONTH('" . getRequest("addtime") . "')";
		$title = date("Y",strtotime(getRequest("addtime"))) . "年" . date("n",strtotime(getRequest("addtime"))) . "月 的搜索词" ;
	}
...
<table border="0" width="96%" align="center" cellpadding="1" cellspacing="1" class="tbtitle">
  <tr>
    <td colspan="4" bgcolor="#F2F4F6"><strong>搜索词列表</strong><span class="fontpadding">条件：<?=$title ?></span></td>
  </tr>
  <tr align="center"  bgcolor="#FFFFE9"  >
   <td width="4%"   >序号</td><td width="8%"  >搜索次数</td>
   <td width="8%"  >时间报表</td>
    <td align="left"  >名称</td>
  </tr>
...
$rs=createGroupPage("count(name) as t,name","@@@searchlog",$condition," order by t desc",$pagesize,$pagenow,$pagetotal,$recordcount);
emptyList($rs,10);
while($rows=fetch($rs))
{
$index++;
?>
  <tr align="center"  bgcolor="#FFFFFF" onMouseMove="tr_onMouseOver(this)" onMouseOut="tr_onMouseOut(this)">
   <td  class="a_fen"><?=$index?></td> <td  class="a_fen"><?=$rows["t"]?></td>
    <td align="center"   class="a_fen"><a href="a_timesearch.php?name=<?=urlencode($rows["name"])?>">详细报表</a></td>
