...
<div class="bodytitle">
	...
	<div class="bodytitletxt">搜索词分析</div>
</div>
 </td>
 </tr>
</table>
...
<table border="0" align="center" width="96%" cellpadding="1" cellspacing="1" class="tbtitle">
  ...
  <tr bgcolor="#FFFFFF">
      <td class="a_fen">
	   搜索词语：
      <input name="name" type="text" id="keyword" value="<?=getRequest("name")?>" style="width:150px">
		<span class="fontpadding">年份选择 ： 
...
$condition.= " and name='" . filterSQL(getRequest("name")) . "'";
$pageurl.="&addtime=" . $addtime ;
$title = "关键字 ". getRequest("name")  . " " . date("Y",strtotime($addtime))  . "年 1月-12月的搜索情况" ;
?><br>
<table border="0" width="96%" align="center" cellpadding="1" cellspacing="1" class="tbtitle">
  ...
  <tr align="center"  bgcolor="#FFFFE9"  >
   <td width="4%"   >月份</td><td width="8%"  >搜索次数</td>
...
<?
$total = fetchValue("select count(*) as v from @@@searchlog $condition",0);
if($total==0)
	$total=1;
$sql = "select month(addtime) as strmonth,count(*) as t from @@@searchlog  $condition group by month(addtime) order by strmonth desc";
$rs = query($sql);
emptyList($rs,10);
while($rows=fetch($rs))
{
