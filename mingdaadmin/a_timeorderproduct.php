...
<div class="bodytitle">
	...
	<div class="bodytitletxt">产品订购分析</div>
</div>
 </td>
 </tr>
</table>
...
$title = "产品 " . fetchValue("select name as v from @@@product where id=".getRequest("pid"),"产品已删除") . " " . date("Y",strtotime($addtime)) . "年 1月-12月的订购的趋势图" ;
?><br>
...
<?
$total = fetchValue("select sum(orderproduct.pnum) as v from `@@@orderproduct` inner join `@@@order` on ( @@@orderproduct.orderid=@@@order.id and @@@orderproduct.pid=".getRequest("pid")."  and YEAR(@@@order.addtime)=YEAR('" . $addtime . "') )",0);
if($total==0)
	$total=1;
$sql = "select month(@@@order.addtime) as strmonth,sum(orderproduct.pnum) as t from `@@@orderproduct` inner join `@@@order` on ( @@@orderproduct.orderid=@@@order.id and @@@orderproduct.pid=".getRequest("pid")."  and YEAR(@@@order.addtime)=YEAR('" . $addtime . "') ) group by strmonth";
//debug($sql);
$rs = query($sql);
emptyList($rs,10);
while($rows=fetch($rs))
