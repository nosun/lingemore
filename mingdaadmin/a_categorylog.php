...
<div class="bodytitle">
	...
	<div class="bodytitletxt">分类关注度</div>
</div>
 </td>
 </tr>
</table>
...
<table width="96%" border="0" cellspacing="1" cellpadding="1" align="center" class="tbtitle"  >
  <tr  bgcolor="#F2F4F6">
    <td    ><a href="?">分类关注度</a> (该功能由前台客户点击分类时统计,<span class="red weight">登入管理员后不列入统计</span>)</td>
  </tr>
</table><br />
<?php
isAuthorize($group[4]);
...
if(getRequest("addtime"))
{
	...
	if(getRequest("timetype")=="y")
	{
		...
		$title = date("Y",strtotime(getRequest("addtime"))) . "年份 的分类访问记录" ;
	}
	...
	else if(getRequest("timetype")=="q")
	{
		...
		$title = date("Y",strtotime(getRequest("addtime"))) . "年" . $arrq[date("n",strtotime(getRequest("addtime")))-1] . " 的分类访问记录" ; 
	}
	else if(getRequest("timetype")=="m")
	{
		$timesql = "YEAR(addtime)=YEAR('" . getRequest("addtime") . "') and MONTH(addtime)=MONTH('" . getRequest("addtime") . "')";
		$title = date("Y",strtotime(getRequest("addtime"))) . "年" . date("n",strtotime(getRequest("addtime"))) . "月 的分类访问记录" ;
	}
...
$condition.=" group by classid" ;
?>
<table border="0" width="96%" align="center" cellpadding="1" cellspacing="1" class="tbtitle">
  <tr>
    <td colspan="4" bgcolor="#F2F4F6"><strong>分类关注度</strong><span class="fontpadding">条件：<?=$title ?></span></td>
  </tr>
  <tr align="center"  bgcolor="#FFFFE9"  >
   <td width="5%"   >序号</td>
   <td width="8%"  >访问次数</td>
    <td align="center"  width="9%"   >详细报表</td>
    <td width="78%" align="left"  >分类名称</td>
  </tr>
...
$rs=createGroupPage("count(classid) as t,classid,max(name) as name","@@@categorylog",$condition," order by t desc",$pagesize,$pagenow,$pagetotal,$recordcount);
emptyList($rs,10);
while($rows=fetch($rs))
{
$index++;
?>
  <tr align="center"  bgcolor="#FFFFFF" onMouseMove="tr_onMouseOver(this)" onMouseOut="tr_onMouseOut(this)">
   <td  class="a_fen"><?=$index?></td>  <td  class="a_fen"><?=$rows["t"]?></td>
    <td align="center"   class="a_fen"><a href="a_timecategory.php?classid=<?=($rows["classid"])?>">详细报表</a></td>
    <td align="left"   class="a_fen"><a target="_blank" href="../products.php?classid=<?=$rows["classid"]?>"><?=$rows["name"]?></a></td>
    </tr>
<?
}
free($rs);
